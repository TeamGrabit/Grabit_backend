...
public class ChallengeCustomRepositoryImpl implements ChallengeCustomRepository {
    ...
    public Optional<Challenge> findChallengeById(Long id) {
        return Optional.ofNullable(jpaQueryFactory.selectFrom(challenge).join(challenge.userChallengeList, userChallenge).fetchJoin().join(userChallenge.user, user).fetchJoin().where(challenge.id.eq(id)).fetchOne());
    }
    ...
    public Page<Challenge> findChallengeBySearchWithPaging(Pageable pageable, String title, String description, String leaderId) {
        ...
        List<Challenge> content = findChallengeQuery.offset(pageable.getOffset()).limit(pageable.getPageSize()).orderBy(challenge.createdAt.desc()).fetch();
    ...
    public Page<Challenge> findUserJoinedChallengeList(Pageable pageable, User u) {
        List<Challenge> challengeList = jpaQueryFactory.selectFrom(challenge).join(challenge.userChallengeList, userChallenge).fetchJoin().where(userChallenge.user.eq(u)).offset(pageable.getOffset()).limit(pageable.getPageSize()).orderBy(challenge.createdAt.desc()).fetch();

        JPAQuery<Challenge> query = jpaQueryFactory.selectFrom(challenge).join(challenge.userChallengeList, userChallenge).fetchJoin().where(userChallenge.user.eq(u));
